...
/**
 * Base directory for everything KAUT keeps outside project repos (stores, engine,
 * workspaces). `KAUT_HOME` overrides it (tests; relocated installs) — unset, the
 * default is `~/.kaut`.
 * @param {Record<string, string|undefined>} [env]
 * @returns {string}
 */
export function kautHome(env = process.env) {
    return env.KAUT_HOME || path.join(homedir(), '.kaut')
}
...
export function discover({ cwd = process.cwd(), env = process.env } = {}) {
    ...
    const engine = env.KAUT_ENGINE || pointer?.engine || path.join(kautHome(env), 'engine')
    const projectId =
