...
function approvalsFor(root, rel, id) {
    const out = []
    const log = storeGit(root, ['log', '--format=%cI%x09%an%x09%s', 'HEAD', '--', rel])
    for (const line of log ? log.split('\n') : []) {
        const [at, author, subject] = line.split('\t')
        const m = subject?.match(/\[owner-approved: ([^\]]+)\]/)
        if (m && m[1].split(',').map((s) => s.trim()).includes(id))
            out.push({ author: ENGINE_AUTHORS.has(author) ? ownerName() : author, at })
    }
    return out
}

// gitstore sets "KAUT" today; "Atlas" is the same engine's pre-rename identity in older stores
const ENGINE_AUTHORS = new Set(['KAUT', 'Atlas'])

/**
 * Store commits are authored under the engine identity ("KAUT" — gitstore sets it), so the
 * approving PERSON behind an owner-gate landing is the operator's own git identity: an
 * `--approve` only ever runs from the owner's environment. Resolved once per process.
 * @returns {string}
 */
let ownerNameCache
function ownerName() {
