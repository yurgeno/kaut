...
export function serializeFrontmatter(fields, body, order = []) {
    // OKF conformance at the single serialization chokepoint: a doc written with an `id` but no
    // explicit `type` gets `type` = its layer (the id's first segment). Duplicates
    // indexgen.typeForId — importing it here would cycle (indexgen imports this module).
    if (typeof fields.id === 'string' && !('type' in fields))
        fields = { ...fields, type: String(fields.id).split('/')[0] }
    const keys = [
